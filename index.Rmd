...
Print alpha. 40% are hits. This is too high (will change when we fix the variances)
...
Get the individual probabilities for each artist. See that the histogram shows likelihood of each song in hit or not hit group. Then identify the keys for each artist with higher than 90 percent chance of being a hit. Animal Collective has 4 keys with higher than a 90% chance of being a hit.
```{r}
...
```
For a single artist, we can pull the probability of a hit in each key.
```{r}
### Velvet Underground
...
Notice substantial variation between each key for each artist. Our first Bayesian model assumed that the mean is the same in each key, but hiearchical bayes will allow us to have a different data situation for each key. 
...
One observation for song i in group (key) j is represented by Yij, where yijs are normally distrubted within each group. N(mu, sigma), allows a different mean for every single group. Muj is the key effect, where the variance in the key is attributable to that key (certain keys are more popular than others).

If we analyzed each key separately, that would assume that each key has nothing to do with one another. This model, however, allows us the parameterize the actual variation between each key which is shared information between the keys (due to artist affect). The sahred information is reflected in the common prior on each key within each artist. Assuming a common prior for the group specific means, each of the key effects are normally distributed around some global overal mean for the artist. Tau squared represents variation across keys, and sigma squared represents the variance of songs within that key. We model two levels of uncertainty in the group data structure.
