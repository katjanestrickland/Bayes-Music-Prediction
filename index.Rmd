...
head(merged_new)
```
We can visualize popularity across keys for this group of artists. Are these differences due to listener demand or due to artist intend? Are Nirvana fans just more sad, and the Eminor key will be more popular than the E minor key is popular for say, Beyonce? Nirvana also wrote more songs in the keys of Csharp and Gsharp than any other key. However, their most popular songs are in more experimental keys, such as Dsharp minor and F minor, and E minor. This begs the question of what we come to expect from a band, and how song writing works for bands. The Velvet Underground only has one song in F minor, but it is their most popular (the song Sunday Morning, off of Velvet Underground and Nico). But The Velvet Underground wrote the most songs in the key of D major, and there is not much variability in this key. Could it be because they are less inclined to take risks in that key? The final section will touch on this concept of hierarchical variance. First, though, let’s use Bayesian prediction to predict hit tracks.
...
The EM algorithm has 5 variance components. The alpha is the probability that any particular data point is from the 1 component, where being from the 0 component is 1-alpha. The five unknown parameters are given by

$$\begin{equation}
\theta = (\mu_1, \mu_0, \sigma_1^2, \sigma_0^2, \alpha)
\end{equation}$$

Estimating theta is difficult because we don't know which of the two components each observation came from. If we had the missing data, we could seperate these units out into the two components. Since we don't observe the data generation process, however, this is a missing data problem.

Consider the missing data as an indicator. Each unit (song) has an indicator, equal to 1 if the observtion came from group 1, or 0 if it came from group 0, where alpha is the probability, because alpha is the proportion of observations that are in group 1 and 0. As we add more "carefully chosen parameters" (i.e. these indicators) then estimating the model becomes easier. 

The likelihood for the EM can be written as a function of both the observed data and the missing data, where Y is the observed data, I is missing data, given theta, the parameters. Observed and missing data are both conditioned on theta, as opposed to only observed conditioned on theta, which is the observed data likelihood.

$$\begin{equation}
P(Y, I | \theta)
\end{equation}$$

This likelihood is not actually observable, because we don't have the value of indicators. We can't use the actual values, but we can plug in their expected values. Instead of plugging in i to the actual likelihood, we'll plug in y hat, the expected value of the indicators. The expected value of an indicator is the probability of that observation (song) being 1. 

The EM algorithm is suited for situations where we don't know which of the two components (non-hit or hit) the data came from. In the first part, the expectation step, the expected indicator for every single observation (whether the observation is in the popular or un-popular group) is calculated. The following step (M step) takes these indicator values from the E step, and outputs new values for the parameters. In other words, the E step takes in current parameter values and outputs estimated values for the indicators. The M step takes in values for the indicators and outputs values for the parameters.

The algorithm code: This is an iterative algorithm, not just one step. We either have to invent these values, start with some unknown theta, then run the M step using values from the E step. The point of the M step is to give better values for the E step. Every step improves the estimates of the unknown. As a hill climbing algorithm, this is guaranteed to find a local mode, not a global mode. 
