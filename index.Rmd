...
Data from streaming platforms offers some hope in modelling future cash flows from new releases, beginning with prediction of a new song's popularity on a streaming platform. Here, I approach the question from a Bayesian perspective. The prediction centers around how a song written in a certain musical key will perform in Spotify's popularity metric. Bayesian methods are particularly suited to model uncertainty, and perform well by incorporating prior information via marginal and conditional probability models. Bayesian methods perform even better when we model this prior information hierarchically. An analysis of this type might appeal to the creative types- what makes music 'experimental?'- yet will also satisfy a financial itch- is a new song in a new key going to sell?

## Intro to the Spotify API - Keys and Popularity Index

Using the Spotify API, I pulled the discographies of 15 bands. These bands are a contrived sample, used only for the purpose of analysis. Data includes 40 measured variables per track, such as album name, track name, duration, and tempo.
...
"Every art fights the noose of verbal description", writes Alex Ross, the New Yorker music critic, in his expansive music history, The Rest is Noise, where, despite spending a career in describing the creative process, he is quick to acknowledge the difficulty of effectively evaluating this deep and difficult art. Challenging might it be to pin down musical creativity, Ross still admits that songs, at their core, can be broken down into certain foundational building blocks.

Keys are one of these foundational concepts. A key is the pattern of intervals between notes, from half tone to semi tone (think of the jump from a black to a white key on a keyboard).  Keys are named with letter names, names that denote the tonic, or the center, of that key. The key of A for instance, is centered around the A tonic, and the key of F is centered around the F tonic. The human ear is accustomed to natural intonations in notes, and the intervals between the notes and the pattern of flats and sharps influence how a key is structured. The key of C, for instance, contains no flats or sharps. The key of C is often considered one of the more basic, or normal, keys for this reason. Keys have also traditionally been associated with emotions, especially in classical music. However, the advent of equal temperament tuning has largely put the debate about key differences to rest. Musicians can transpose any song into any key, which they might do in order to match the singer's intended tone or create a certain timbre in the overall sound.

Chord structure matters more to the emotion and story of a song. Just like lyrics, carefully chosen chords move inside the key and change the trajectory of a song. A popular pop progression is the 145, such as in the song "Baby Shark", which became global earworm in 2019. Baby Shark is written in the key of C with a 145 chord progression, and has a predictable pathway and calming resolution.

In the absence of chord structure data, I turn to a key analysis. Despite the verdict that key differences at the emotive level are moot, we still might gain insight from key differences at the popularity level. Especially because, from a look at the descriptive statistics around popularity and key, there looks to be significant variance both between and within artists.

Starting with the raw data, the distribution of keys for our sample of artists is predictable: keys of G major, D major and C major (more simple keys with less sharps and flats) are represented the most. The distribution of keys doesn't hold necessarily across artists, however. The experimental band Radiohead wrote more songs in the key of Asharp than Animal Collective, for instance, who write more songs in the Key of D. This begs the question of how hits are formed among artist. Does a band get better by writing a lot in one key? Could other bands be more likely to land on a hit when they write outside of their most frequented key?
...
head(merged_new)
```
Next, we can visualize popularity across keys for this group of artists. Does listener demand or artist intend drive the interaction between key popularity and usage? Maybe Nirvana fans just more "down". In that case, might the Eminor key would be more popular than the Eminor key among Beyonce fans? Nirvana also wrote more songs in the keys of C and Gmajor than any other key. However, their most popular songs are in more experimental keys, such as D#minor, Fminor, and E minor. This begs the question of what we come to expect from a band, and what turns out to be popular among fans. The Velvet Underground only has one song in Fminor, but the song (Sunday Morning, off of Velvet Underground and Nico) is their most popular. The Velvet Underground wrote the most songs in the key of Dmajor, and there is not much variability in this key. Could it be because they are less inclined to take risks in that key? The final section will touch on this concept of hierarchical variance. First, though, let’s use Bayesian prediction to predict hit tracks from keys.
...
I fit a two-component mixture model to the distribution of popularity averages, one component that represents songs with average popularity, and another component that represent hit tracks (high popularity). We don't observe the grouping of songs in the data generation process (i.e. Spotify doesn't have a 'hit track' variable); therefore, this is considered a "missing data" problem. The Expectation-Maximization (EM) algorithm will identify which of the two components the data came from, and since the distribution of popularity appears to be normal for both components, then we choose a normal-normal mixture model.
...
The model consists of 5 components: the $\mu$'s (means) of the two groups, the $\sigma^2$'s (variances) of the two groups, and $\alpha$, representing the probability that any particular data point (song) is from the 1 (hit track) component. A point from the 0 component is represented by 1-$\alpha$. The five unknown parameters are given by

$$\begin{equation}
\theta = (\mu_1, \mu_0, \sigma_1^2, \sigma_0^2, \alpha)
\end{equation}$$

Estimating $\theta$ proves difficult, given that we can't tell which of the two components each observation came from. If we had the missing data, we could seperate units into their respective components, but instead, we'll consider the missing data as an indicator. Each unit (song) has an indicator, equal to 1 if the observation (that song) came from group 1, and equal to 0 if the observation came from group 0. $\alpha$ is the proportion of observations that are in group 1 and 0. In a counter-intuitive concept to much of statistical modeling, adding more "carefully chosen parameters" (i.e. these indicators) improves model estimation.

The likelihood for the EM is written as a function of both the observed data and the missing data, where Y is the observed data, I is missing data. Observed and missing data are both conditioned on theta, as opposed to only observed variables conditioned on $\theta$, which is the observed data likelihood.
...
Plots for the course of the parameters is mapped below. Most of the gains in the observed data likelihood appear in the first few iterations, and after that, parameter estiamtes flatten out until the model converges. Estimates result in an overall maximum log-likelihood (=296.61) with $\mu_0$ at 0.18, $\mu_1$ at .40., and $\alpha$ at 0.40. The optimal kind of mixture components for this distribution is further mapped on to the MLE Densities histogram, where the green distribution represents hit tracks, and the red distribution, non-hit tracks.

This model troubingly assigns a high probability to a song being a hit (at .40). Also notice the overlap in distributions. Songs that never reach popularity over 0.2 have a probability of being in the hit track group. The flexibility built into the model (from assigning unique variances per group) causes this overlap. The green group (hit songs) doesn't just have a higher mean, it also has a greater variance. Contraining the variances equal will help address this overlap.
...
We can also identify the keys for each artist with higher than 90 percent chance of being a hit. Animal Collective has 4 keys with higher than a 90% chance of being a hit. Although these songs are lower average popularity than other artists, incorporating the priors in a Bayesian way demonstrates their likelihood at hits within this band.
...
Notice substantial variation in popularity of keys for each artist. Our first Bayesian model assumed that the mean of popularity is the same across each key for each band, but hiearchical bayes will allow us model a different data situation for each key for each band. Staying in the normal family, we will think more closely about which parameters are shared or not shared between and within groups. 
...
One observation for song i in key j is represented by $Y_{ij}$, where $y_{ij}$ are normally distributed within each group. $N(\mu, \sigma^2)$ allows a different mean for every single group. $\mu_{j}$ is the "key effect", where the variance in the key is attributable to that key (certain keys are more popular than others).
...
Assume a known $\sigma^2$ and model the parameters $y_{ij}$ as the observations i in group j and mu as the global mean:
...
When calculating the posterior distributions for each key, we also note extremes of $\tau^2$. As $\tau^2$ goes to infinity, then there is infinite variation between keys. In that case, we might as well model all of the keys seperately, getting the key specific effect for each. In that case, we'd center $\mu_j$ at the data, analyzing every key seperately. On the other end of the extreme, $\tau^2$ goes to 0, and this means that there is almost no variation between keys, so every key has the same effect. In reality, $\tau^2$ will be somewhere not right at 0, or close to infinity It is a compromize between the group specific and the global mean.

In our situation, sigma squared is known, and we will estimate the posterior by getting samples from the posterior, where the sufficient stats are just the group means. The code takes 1000 samples from the joint posterior. For each of these samples, we first sample $\tau^2$ from the grid, and that Tau squared, loop through a sample of each of the group specific means. Small values of Tau squared, indicate pulling of the group specific mean towards a global mean (shared information between keys).
...
Below, a posterior comparison between keys. Posterior probability that group 5 is greater than group 6, that group 2 greater than group 1:
...
Using this common prior distribution, and sharing between groups, the estimates of the within group parameters are shrunk towards the global mean. Estimates (red dots) are pulled in from the data means (black dots). For group 5, the key looks popular, but the red means not as popular as we might think. Keys that are closer to the global mean don't move as much, but extremes are pulled more.
...
Sample new data point's value, from the model, which is normal for the data distribution, centered with $\sigma^2$, using the sample of j that we already saw. This is like adding a song to an existing key, and evaluating the distribution of its posterior popularity.
...
Where the grid method for evaluating variance fails beyond three parameters, MCMC (Markov Chain Monte Carlo) saves the day. MCMC harnesses the ability of optimization algorithms, such as EM, to move around the posterior space and find optimal grid values, specifically builds on each previous iteration. Using MCMC, we establish a path through the posterior space instead of randomly sampling through any one of the posteriors at a given time. Given our entire path, the current sample is based on the previous sample, and the distribution is based on the immediate previous sample. 

The workhorse of the Monte Carlo is the Gibbs Sampler, which samples from the joint posterior distribution by iteratively sampling from each distribution of each parameter, given another parameter. Here, we start with some arbitrary values, and from those values, sample from the conditional distribution given the other values of each parameter. Given that current sample, we sample the next iteration, and cycle through all the parameters until $\theta$ is sampled from the last parameter. 
...
The dependence on the previous sample that Markov Chain relies upon is both a blessing and a curse. Since the algorithm moves based on the last move (as in the case of a random walk), we can climb the hill of the posterior distribution from any starting point. However, dependence on prior values is an obvious tradeoff, in that it might result in autocorrelation of estimates. To address this issue, we manually evaluate convergence by visualizing where the chains come together, and then reduce autocorrelation of the paraemter values in each chain. 

In the sample below, the GIBBS samples thetas for 10,000 iterations. We establish convergence, seeing whether the black and red lines "come together", which here takes about 200 iterations. The first 100 values are thrown-out as part of the burn-in sample.
